...
pub use oumi_integration::{
    ActionType, MarketAnalysis, MarketData, MarketTrend, OumiConfig, OumiEngine, PredictionType,
    RecommendedAction, RiskLevel, TradingPrediction, Urgency,
};

pub use opensearch_integration::{
    AnalyticsConfig, AnomalyResult, AnomalyType, IndexConfig, OpenSearchConfig, OpenSearchEngine,
    PatternAnalysis, PatternType, PerformanceConfig, RiskIndicator, RiskSeverity, SearchResult,
    VectorSearchConfig,
};

pub use lmcache_integration::{
    CacheStats, CachedInference, EvictionPolicy, InferenceResult, LMCacheConfig, LMCacheEngine,
};
...
impl AICoordinator {
    ...
    pub async fn get_combined_prediction(
        &self,
        token_address: &str,
        market_data: &MarketData,
    ) -> Result<CombinedPrediction> {
        debug!(
            "🤖 Generating combined prediction for token: {}",
            token_address
        );
        ...
        let oumi_prediction = if self.config.oumi.enabled {
            match self
                .oumi_engine
                .predict_token(token_address, market_data)
                .await
            {
                Ok(pred) => Some(pred),
                Err(e) => {
        ...
        let opensearch_patterns = if self.config.opensearch.enabled {
            ...
            let query_vector: Vec<f32> =
                vec![0.5; self.config.opensearch.vector_dimensions as usize];

            match self
                .opensearch_engine
                .search_similar_patterns(&query_vector, None)
                .await
            {
        ...
        // Combine predictions
        let combined = self
            .combine_predictions(oumi_prediction, &opensearch_patterns, &anomalies)
            .await?;
    ...
    async fn combine_predictions(
        ...
        if !opensearch_patterns.is_empty() {
            let avg_pattern_score = opensearch_patterns.iter().map(|p| p.score).sum::<f64>()
                / opensearch_patterns.len() as f64;
            confidence_scores.push(avg_pattern_score / 10.0); // Normalize to 0-1
        }

        // Anomaly impact on confidence
        let anomaly_impact = anomalies
            .iter()
            .map(|a| a.severity)
        ...
        let risk_assessment = self
            .assess_risk(&oumi_prediction, anomalies, final_confidence)
            .await;
        ...
        if let Some(pred) = oumi_prediction {
            if pred.confidence > self.config.integration.confidence_threshold {
                ...
                return RecommendedAction {
                    ...
                    urgency: if pred.confidence > 0.9 {
                        Urgency::High
                    } else {
                        Urgency::Medium
                    },
    ...
    pub async fn analyze_wallet(&self, wallet_address: &str) -> Result<PatternAnalysis> {
        ...
        self.opensearch_engine
            .analyze_wallet_behavior(wallet_address)
            .await
    }
}
