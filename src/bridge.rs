...
impl SimpleEventDetector {
    ...
    pub fn detect_from_parsed(&self, signature: &str, program: &str, accounts: &[String]) -> Vec<BridgeEvent> {
        let mut events = Vec::new();
        let timestamp = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
...
impl SimpleEventProcessor {
    ...
    pub async fn process_event(&mut self, event: &BridgeEvent) -> Result<ProcessingResult, ProcessingError> {
        self.processed_count += 1;
        
        match &event.event_type {
            EventType::DexTransaction { signature, program, .. } => {
                info!("🔄 Processing DEX transaction: {} on {}", signature, program);
            ...
            EventType::LargeSwap { amount, token_in, token_out } => {
                info!("💰 Processing large swap: {} {} → {}", amount, token_in, token_out);
            ...
            EventType::NewPool { token_mint, initial_liquidity } => {
                info!("🆕 Processing new pool: {} with {} liquidity", token_mint, initial_liquidity);
