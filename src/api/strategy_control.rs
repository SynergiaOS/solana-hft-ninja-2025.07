...
impl StrategyController {
        ...
        if let Some(strategy) = strategies.get_mut(strategy_name) {
            match command {
                ...
                StrategyCommand::UpdateConfig(_config) => {
                    ...
                    Ok(format!(
                        "Strategy '{}' config updated successfully",
                        strategy_name
                    ))
                }
            }
    ...
    pub async fn emergency_stop(&self) -> String {
        ...
        format!(
            "Emergency stop executed - {} strategies disabled",
            stopped_count
        )
    }
}
...
) -> impl Filter<Extract = (Arc<StrategyController>,), Error = std::convert::Infallible> + Clone {
    warp::any().map(move || controller.clone())
}

/// Handle list all strategies
async fn handle_list_strategies(
    controller: Arc<StrategyController>,
...
async fn handle_get_strategy(
    ...
    if let Some(strategy) = controller.get_strategy(&name).await {
        ...
        Ok(warp::reply::json(
            &serde_json::json!({"error": "Strategy not found"}),
        ))
    }
}
