...
impl JitoExecutor {
    ...
    pub async fn execute_bundle(
        ...
        info!(
            "📦 Submitted bundle: {} with {} transactions",
            bundle_id,
            bundle_txs.len()
        );
    ...
    fn create_tip_transaction(&self, tip_amount: u64) -> Result<Transaction> {
        ...
        let instruction =
            system_instruction::transfer(&self.tip_keypair.pubkey(), &tip_account, tip_amount);
    ...
    fn calculate_tip_amount(&self, transactions: &[BundleTransaction]) -> u64 {
        ...
        let avg_priority: u8 =
            transactions.iter().map(|tx| tx.priority).sum::<u8>() / transactions.len() as u8;
...
pub fn create_bundle_transaction(transaction: Transaction, priority: u8) -> BundleTransaction {
    BundleTransaction {
        transaction,
        priority,
        max_retries: 3,
...
pub fn create_high_priority_bundle_transaction(transaction: Transaction) -> BundleTransaction {
    BundleTransaction {
        transaction,
        priority: 255,
        max_retries: 5,
