...
use solana_hft_ninja::{config::Config, utils::{logging::init_logging, metrics::MetricsServer}};
...
async fn main() -> Result<()> {
    ...
    // Execute command
    match cli.command {
        Commands::Trade { strategies, dry_run } => {
            run_trading_engine(config, strategies, dry_run).await
        }
        Commands::Api { port } => {
            run_api_server(config, port).await
        }
        Commands::Data { sources } => {
            run_data_collector(config, sources).await
        }
        Commands::Backtest { strategy, start_date, end_date } => {
            run_backtest(config, strategy, start_date, end_date).await
        }
        Commands::Benchmark { bench_type, iterations } => {
...
async fn run_benchmark(config: Config, bench_type: String, iterations: u32) -> Result<()> {
    info!("⚡ Running {} benchmark with {} iterations", bench_type, iterations);
...
async fn validate_setup(config: Config) -> Result<()> {
    ...
    let solana_client = solana_hft_ninja::network::SolanaClient::new(&config.solana.rpc_url).await?;
