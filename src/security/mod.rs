...
impl SecurityManager {
    ...
    pub async fn validate_transaction(&mut self, transaction: &TransactionRequest) -> Result<SecurityValidation> {
        ...
        if self.circuit_breaker.is_open() {
            ...
            return Ok(SecurityValidation::Blocked("Circuit breaker active".to_string()));
        }
        
        // Check risk limits
        if !self.risk_limits.check_position_size(transaction.amount_sol)? {
            warn!("🚨 Position size exceeds limit: {} SOL", transaction.amount_sol);
            return Ok(SecurityValidation::Blocked("Position size limit exceeded".to_string()));
        }
        
        // Check daily loss limit
        if !self.risk_limits.check_daily_loss_limit().await? {
            warn!("🚨 Daily loss limit exceeded");
            return Ok(SecurityValidation::Blocked("Daily loss limit exceeded".to_string()));
        }
        
        // Validate wallet security
        if !self.wallet_security.validate_transaction(transaction).await? {
            error!("🚨 Wallet security validation failed");
            return Ok(SecurityValidation::Blocked("Wallet security validation failed".to_string()));
        }
        
        let validation_time = start_time.elapsed().unwrap_or(Duration::from_millis(0));
        
        info!("✅ Transaction security validation passed ({:.2}ms)", validation_time.as_millis());
    ...
    pub fn get_security_status(&self) -> SecurityStatus {
        SecurityStatus {
            ...
            last_security_check: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
        }
    }
}
