...
impl WalletSecurity {
    ...
    pub async fn validate_transaction(&self, transaction: &TransactionRequest) -> Result<bool> {
        ...
        if let Ok(elapsed) = state.last_activity.elapsed() {
            let timeout = std::time::Duration::from_secs(self.config.session_timeout_minutes as u64 * 60);
            if elapsed > timeout {
    ...
    pub async fn get_wallet_status(&self) -> WalletStatus {
        ...
        WalletStatus {
            ...
            hardware_wallet_connected: self.hardware_wallet.as_ref().map(|hw| hw.connected).unwrap_or(false),
            last_activity: state.last_activity.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs(),
        }
    }
}
