...
impl LMCacheEngine {
    ...
    async fn load_persisted_cache(&self) -> Result<()> {
        ...
        debug!("🧠 Loading persisted cache from: {}", self.config.persistence_path);
        Ok(())
    }
    ...
    async fn evict_entries(&self, cache: &mut HashMap<String, CachedInference>) {
        ...
        match self.config.eviction_policy {
            EvictionPolicy::LRU => {
                ...
                let keys_to_remove: Vec<String> = entries.iter().take(evict_count).map(|(k, _)| k.to_string()).collect();
    ...
    async fn update_stats(&self, hit: bool, start_time: SystemTime) {
        ...
        if let Ok(duration) = start_time.elapsed() {
            ...
            stats.average_response_time_ms = 
                (stats.average_response_time_ms * (stats.total_requests - 1) as f64 + response_time) 
