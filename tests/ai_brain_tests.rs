...
use solana_hft_ninja::ai::{
    AIConfig, AICoordinator, ActionType, EvictionPolicy, LMCacheConfig, LMCacheEngine, MarketData,
    OpenSearchConfig, OpenSearchEngine, OumiConfig, OumiEngine, PredictionType, RiskLevel,
    TradingPrediction, Urgency,
};
...
async fn test_ai_coordinator() -> Result<()> {
    ...
    let market_data = create_test_market_data();
    let combined_prediction = coordinator
        .get_combined_prediction("test_token", &market_data)
        .await?;

    // Verify combined prediction structure
    assert!(
        combined_prediction.final_confidence >= 0.0 && combined_prediction.final_confidence <= 1.0
    );
    assert!(combined_prediction.oumi_prediction.is_some());
    assert!(matches!(
        combined_prediction.recommended_action.action_type,
        ActionType::Buy
            | ActionType::Sell
            | ActionType::Hold
    ...
    assert!(matches!(
        combined_prediction.risk_assessment.overall_risk,
...
async fn test_ai_performance() -> Result<()> {
    ...
    println!(
        "📊 Cache stats: {} hits, {} misses, {:.2}% hit ratio",
        stats.hits,
        stats.misses,
        hit_ratio * 100.0
    );
...
async fn test_trading_scenario() -> Result<()> {
    ...
    let prediction = coordinator
        .get_combined_prediction("bullish_token", &bullish_data)
        .await?;
    ...
    let prediction = coordinator
        .get_combined_prediction("bearish_token", &bearish_data)
        .await?;
